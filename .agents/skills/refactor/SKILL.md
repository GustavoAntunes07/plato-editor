...
3. **AI-DLC state is your safety net** - verify the intent, branch, and working tree before editing
...
- For implementation refactors, work from an intent and verify the intent branch before editing.
- Do not create or switch branches automatically; ask the developer or use the AI-DLC branch command.
- Preserve unrelated user changes.
- Run project validation commands before calling the refactor complete.
...
1. PREPARE
   ...
   - Confirm the active intent when the refactor changes files
   - Run `bun aidlc:branch <intent-id>` for AI-DLC work
...
5. CLEAN UP
   ...
   - Suggest a commit message only if the developer asks
```
